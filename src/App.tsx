...
import { RestaurantOrderNotifier } from "@/components/RestaurantOrderNotifier";
...
const AppContent = () => {
  ...
  return (
    <div className="relative flex flex-col h-screen w-full max-w-md mx-auto bg-background shadow-2xl overflow-hidden border-x border-border/5 select-none">
      {/* Global Restaurant Order Notifications */}
...
const App = () => {
  ...
  const refreshFromServer = useUserStore((state) => state.refreshFromServer);
  const isAuthenticated = useUserStore((state) => state.isAuthenticated);
  const userId = useUserStore((state) => state.userId);
  const managedRestaurants = useUserStore((state) => state.managedRestaurants);
  const fetchAllRelevantOrders = useOrderStore((state) => state.fetchAllRelevantOrders);
  ...
  // Refresh user data from server on app load to sync latest changes
  // Also fetch all their relevant orders (as a buyer and as a restaurant)
  useEffect(() => {
    if (isAuthenticated && userId) {
      refreshFromServer();
      
      const restIds = [`user-restaurant-${userId}`];
      if (managedRestaurants) {
        managedRestaurants.forEach(r => restIds.push(r.id));
      }
      fetchAllRelevantOrders(userId, restIds);
    }
  }, [isAuthenticated, userId, managedRestaurants, refreshFromServer, fetchAllRelevantOrders]);
