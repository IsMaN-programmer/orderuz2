...
export default function Orders() {
  ...
  const clearReviewPrompt = useOrderStore((state) => state.clearReviewPrompt);
  const [selectedOrder, setSelectedOrder] = useState<Order | null>(null);
  const [trackingOpen, setTrackingOpen] = useState(false);
  const [reviewOrder, setReviewOrder] = useState<Order | null>(null);

  // Track previously seen completed order IDs to detect new completions
  const [seenCompletedOrders, setSeenCompletedOrders] = useState<Set<string>>(new Set());

  // Auto-open ReviewModal when order completes (after timer finishes)
  useEffect(() => {
    // Find a newly completed order that hasn't been reviewed and wasn't seen before
    const newlyCompletedOrder = orders.find(
      order => order.status === 'completed' && !order.reviewId && !seenCompletedOrders.has(order.id)
    );
    
    if (newlyCompletedOrder && !reviewOrder) {
      setReviewOrder(newlyCompletedOrder);
      setSeenCompletedOrders(prev => new Set([...prev, newlyCompletedOrder.id]));
      if (newlyCompletedOrder.reviewPromptPending) {
        clearReviewPrompt(newlyCompletedOrder.id);
      }
    }
  }, [orders, reviewOrder, seenCompletedOrders, clearReviewPrompt]);

  // Filter and sort orders - newest first
  const filteredOrders = orders
    .filter(order => order.userId === user.userId) // only show the user's purchased orders
    .filter(order => 
      activeTab === 'active' 
          ...
          {!isHistory ? (
            ...
            <>
              <button
                ...
                className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-bold hover:bg-primary/90 transition"
              >
              ...
              {order.status === 'completed' && onReview && (
                ...
                >
                  ...
                  {order.reviewId ? 'Edit Review' : 'Review'}
                </button>
              )}
            </>
          )}
