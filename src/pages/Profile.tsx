...
import { useSharedVideoStore, SharedVideo } from '@/store/sharedVideoStore';
...
import { RESTAURANT_TYPES } from '@/pages/Search';

export default function Profile() {
  const ADMIN_EMAIL = 'sr.racer77@gmail.com';
  const { t, language, tNested } = useTranslation();
  const user = useUserStore();
  const interactions = useVideoInteractionsStore((state) => state.interactions);
  const getTotalLikesForVideos = useVideoInteractionsStore((state) => state.getTotalLikesForVideos);
  const getTotalViewsForVideos = useVideoInteractionsStore((state) => state.getTotalViewsForVideos);
  ...
  const addSharedVideo = useSharedVideoStore((state) => state.addVideo);
  const deleteSharedVideo = useSharedVideoStore((state) => state.deleteVideo);
  const fetchSharedVideos = useSharedVideoStore((state) => state.fetchVideos);
  const getVideosByOwner = useSharedVideoStore((state) => state.getVideosByOwner);
  const updateSharedVideo = useSharedVideoStore((state) => state.updateVideo);

  useEffect(() => {
    fetchSharedVideos();
  }, [fetchSharedVideos]);
  
  // Order store for restaurant orders
  const orders = useOrderStore((state) => state.orders);
  const confirmOrder = useOrderStore((state) => state.confirmOrder);
  const updateOrderStatus = useOrderStore((state) => state.updateOrderStatus);
  
  // Get restaurant IDs for filtering orders - include managed restaurant IDs, fallback ID, and IDs from uploaded videos
  const restaurantIds = useMemo(() => {
    ...
    // Also add the fallback ID pattern used when no managed restaurant exists
    ids.push(`user-restaurant-${user.userId}`);
    // Also add restaurant IDs from videos uploaded by this user
    ...
    return ids;
  }, [user.managedRestaurants, user.userId, sharedVideos]);
  ...
  const confirmedRestaurantOrders = useMemo(() => {
    return orders.filter(o => 
      restaurantIds.includes(o.restaurantId) && 
      o.confirmedByRestaurant &&
  ...
  const completedRestaurantOrders = useMemo(() => {
    return orders.filter(o => 
      restaurantIds.includes(o.restaurantId) && 
      ['completed', 'cancelled'].includes(o.status)
  ...
  const postedVideos = useMemo(() => {
    if (!user.userId) return [];
    const liveVideos = sharedVideos.filter((v) => v.ownerId === user.userId);
    // Use cached user.restaurantVideos if sharedVideos hasn't loaded yet to prevent UI flash/delay
    if (liveVideos.length === 0 && user.restaurantVideos && user.restaurantVideos.length > 0) {
      return user.restaurantVideos;
    }
    return liveVideos;
  }, [sharedVideos, user.userId, user.restaurantVideos]);

  // Calculate total views and likes from video interactions store
  const videoIds = useMemo(() => {
    return postedVideos.map(v => v.id);
  }, [postedVideos]);
  
  const totalVideoViews = useMemo(() => {
    const liveViews = getTotalViewsForVideos(videoIds);
    // Fallback to locally cached views if interaction API hasn't resolved
    return liveViews > 0 ? liveViews : (user.totalVideoViews || 0);
  }, [videoIds, getTotalViewsForVideos, user.totalVideoViews, interactions]);
  
  const totalVideoLikes = useMemo(() => {
    const liveLikes = getTotalLikesForVideos(videoIds);
    // Fallback to locally cached likes if interaction API hasn't resolved
    return liveLikes > 0 ? liveLikes : (user.totalVideoLikes || 0);
  }, [videoIds, getTotalLikesForVideos, user.totalVideoLikes, interactions]);

  // Fetch interactions for posted videos so stats display correctly
  React.useEffect(() => {
    videoIds.forEach(id => {
      fetchInteraction(id);
    });
  }, [videoIds, fetchInteraction]);
  ...
  const [isMyVideosDialogOpen, setIsMyVideosDialogOpen] = useState(false);
  const [isRestaurantOrdersDialogOpen, setIsRestaurantOrdersDialogOpen] = useState(false);
  ...
  const handleAddRestaurantVideo = async () => {
    ...
    // Check file size (max 30MB for base64 storage on server)
    if (videoForm.videoFile.size > 30 * 1024 * 1024) {
    ...
    try {
      ...
      // Convert video to base64 for server storage
      const videoBase64 = await fileToBase64(videoForm.videoFile);

      // Get restaurant info
      const restaurantName = user.managedRestaurants && user.managedRestaurants.length > 0
        ? user.managedRestaurants[0].name
        : user.name || 'Restaurant';
      
      const restaurantId = user.managedRestaurants && user.managedRestaurants.length > 0
        ? user.managedRestaurants[0].id
        : `user-restaurant-${user.userId}`;
    
      const newVideo: SharedVideo = {
        id: videoId,
        videoUrl: videoBase64, // Store as base64 data URL
        ...
        ownerId: user.userId,
        ownerName: user.name,
        ownerProfileImage: user.avatar,
        restaurantId: restaurantId,
        restaurantName: restaurantName
      };
      
      // Save to shared video store (visible to all users)
      await addSharedVideo(newVideo);
      // Keep legacy user profile list in sync (non-blocking for UI if it fails)
      try {
        await user.addRestaurantVideo(newVideo);
      } catch (syncError) {
        console.warn('Failed to sync user restaurantVideos, using shared store as source', syncError);
      }
  ...
  const handleViewRestaurantDetails = (restaurant: any) => {
    ...
    setRestaurantForm({
      ...
      category: restaurant.category || 'National Food',
  ...
  const handleAuth = async () => {
    if (authMode === 'register') {
      ...
      const result = await user.register(authForm.name, authForm.email, authForm.password, selectedAccountType);
      if (!result.success) {
      ...
      const loginSuccess = await user.login(authForm.email, authForm.password, selectedAccountType);
      if (!loginSuccess) {
        toast.error('Invalid email, password, or account type mismatch');
        return;
      }
  ...
  const handleSaveProfile = () => {
    ...
    // Update all user's videos with new profile image and name
    const userVideos = getVideosByOwner(user.userId);
    userVideos.forEach(video => {
      updateSharedVideo(video.id, {
        ownerName: editForm.name,
        ownerProfileImage: editForm.avatar
      });
    });
  ...
  const handleDeleteAccount = async () => {
    ...
    const success = await user.deleteAccount(deleteAccountPassword);
    if (success) {
  ...
  const handleAdminResetAllData = async () => {
    if (!isAdminUser) {
    ...
    try {
      const result = await api.auth.resetAllData({ email: ADMIN_EMAIL, password: adminResetPassword });
      if (!result.success) {
    ...
    <div className="h-screen bg-background pb-24 overflow-y-auto flex flex-col">
      ...
      >
        ...
        <motion.div 
          ...
          className="container mx-auto px-4 mt-1 space-y-6 pb-8"
        >
          {/* Stats Cards */}
          <div>
            <div className="grid grid-cols-3 gap-3">
              {(user.accountType === 'restaurant' ? [
                { label: 'Video Views', value: formatNumber(totalVideoViews), icon: Eye },
                { label: 'Video Likes', value: formatNumber(totalVideoLikes), icon: ThumbsUp },
                { label: 'Videos', value: formatNumber(postedVideos.length), icon: Play },
              ] : [
                { label: t('orders_count'), value: formatNumber(user.ordersCount), icon: History },
                { label: t('favorites'), value: formatNumber(user.followedRestaurants.length), icon: Heart },
                { label: t('saved'), value: formatNumber(user.savedCount), icon: Bell },
              ]).map((stat, i) => (
            ...
            <section>
              ...
              <section className="mt-6">
                ...
                <Card className="bg-card/50 backdrop-blur border-border/40">
                  <button 
                    onClick={() => setIsMyVideosDialogOpen(true)}
                    className="w-full flex items-center justify-between p-4 hover:bg-muted/50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {postedVideos.length || 0} videos posted
                        </p>
                      </div>
                    </div>
                    <ChevronRight className="w-5 h-5 text-muted-foreground" />
              ...
              <section className="mt-6">
                ...
                <Card className="bg-card/50 backdrop-blur border-border/40">
                  <button 
                    onClick={() => setIsRestaurantOrdersDialogOpen(true)}
                    className="w-full flex items-center justify-between p-4 hover:bg-muted/50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-xl bg-orange-500/10 relative">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {pendingRestaurantOrders.length > 0 
                            ? `${pendingRestaurantOrders.length} pending confirmation`
          ...
          <div className="space-y-4">
            ...
            <div className="overflow-hidden">
              ...
              <Input 
                type="file"
                accept="video/*"
                className="w-full file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-muted"
              ...
              <p className="text-xs text-muted-foreground mt-1 truncate max-w-full">
                {videoForm.videoFile ? `Selected: ${videoForm.videoFile.name.length > 30 ? videoForm.videoFile.name.slice(0, 30) + '...' : videoForm.videoFile.name}` : 'No video selected'}
              </p>
            </div>
            <div className="flex gap-2">
              <Button 
                ...
                className="flex-1"
                disabled={isUploadingManager}
              >
                {isUploadingManager ? 'Uploading...' : 'Upload Video'}
              </Button>
              <Button 
                variant="outline"
      ...
      {/* My Videos Dialog */}
      <Dialog open={isMyVideosDialogOpen} onOpenChange={setIsMyVideosDialogOpen}>
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>My Videos</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            {postedVideos.length > 0 ? (
              postedVideos.map((video) => (
                <Card key={video.id} className="bg-card/50 backdrop-blur border-border/40 overflow-hidden">
                  <div className="flex gap-3 p-3 w-full">
                    <div 
                      className="relative w-20 h-20 rounded-lg overflow-hidden bg-muted flex-shrink-0 cursor-pointer"
                      onClick={async () => {
                        try {
                          // New uploads use base64/http URLs; legacy uploads may use IndexedDB IDs
                          if (video.videoUrl.startsWith('data:') || video.videoUrl.startsWith('http')) {
                            setPreviewVideo({ id: video.id, url: video.videoUrl });
                            return;
                          }

                          const { getVideo } = await import('@/lib/videoStorage');
                          const blob = await getVideo(video.videoUrl);
                          if (!blob) {
                            toast.error('Video not found');
                            return;
                          }
                          const url = URL.createObjectURL(blob);
                          setPreviewVideo({ id: video.id, url });
                        } catch (err) {
                    ...
                    <button
                      onClick={async () => {
                        try {
                          await deleteSharedVideo(video.id, user.userId);
                          try {
                            await user.deleteRestaurantVideo(video.id);
                          } catch (syncError) {
                            console.warn('Failed to sync user restaurantVideos on delete', syncError);
                          }
          ...
          <div className="space-y-3">
            ...
            {isAdminUser && (
              <div className="space-y-2 rounded-xl border border-destructive/30 p-3">
                <p className="text-xs text-muted-foreground">
                  Admin: reset all accounts, videos, orders, comments and interactions.
                </p>
                <Input
                  type="password"
                ...
                />
                <Button
                  onClick={handleAdminResetAllData}
                  className="w-full"
                  variant="destructive"
                  disabled={isResettingAllData}
                >
                  {isResettingAllData ? 'Resetting...' : 'Reset All Data'}
                </Button>
              </div>
            )}
          </div>
      ...
      <Dialog open={isRestaurantOrdersDialogOpen} onOpenChange={setIsRestaurantOrdersDialogOpen}>
        <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col p-0">
          <DialogHeader className="flex-shrink-0 p-6 pb-2">
            <DialogTitle>Restaurant Orders</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div>
                <h3 className="text-sm font-semibold text-orange-500 mb-3 flex items-center gap-2">
                  ...
                  Pending Confirmation ({pendingRestaurantOrders.length})
                </h3>
                {pendingRestaurantOrders.length > 0 ? (
                  <div className="space-y-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-primary mb-3 flex items-center gap-2">
                  ...
                  Confirmed Orders ({confirmedRestaurantOrders.length})
                </h3>
                {confirmedRestaurantOrders.length > 0 ? (
                  <div className="space-y-3">
                    {confirmedRestaurantOrders.map((order) => (
                      <RestaurantOrderCard 
                        key={order.id} 
                        order={order}
                  ...
                  <p className="text-sm text-muted-foreground py-4 text-center">No confirmed orders</p>
                )}
              </div>
              ...
              <div>
                <h3 className="text-sm font-semibold text-muted-foreground mb-3 flex items-center gap-2">
                  ...
                  Order History ({completedRestaurantOrders.length})
                </h3>
                {completedRestaurantOrders.length > 0 ? (
                  <div className="space-y-3">
                    {completedRestaurantOrders.slice(0, 10).map((order) => (
                      <RestaurantOrderCard 
                        key={order.id} 
                        order={order}
                        isHistory
                    ...
                    {completedRestaurantOrders.length > 10 && (
                      <p className="text-xs text-muted-foreground text-center py-2">
                        And {completedRestaurantOrders.length - 10} more orders...
                      </p>
                    )}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground py-4 text-center">No order history yet</p>
                )}
              </div>
            </div>
          </div>
...
function ProfileMenuItem({ icon: Icon, label, subLabel, onClick }: ProfileMenuItemProps) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className="w-full flex items-center justify-between p-4 text-left transition-colors cursor-pointer hover:bg-accent/10"
    >
...
interface RestaurantOrderCardProps {
  order: Order;
  onConfirm?: () => void;
  onUpdateStatus?: (orderId: string, status: 'pending' | 'preparing' | 'delivering' | 'completed' | 'cancelled') => void;
  isPending?: boolean;
  isHistory?: boolean;
}

function RestaurantOrderCard({ order, onConfirm, onUpdateStatus, isPending, isHistory }: RestaurantOrderCardProps) {
      ...
      <CardContent className="p-4 space-y-3">
        ...
        {order.videoId && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>From video: {order.videoId.slice(-8)}</span>
          </div>
        )}

        {/* Items */}
        <div className="space-y-1 py-2 border-y border-border/30">
          {order.items.map((item, idx) => (
            <div key={idx} className="flex justify-between text-sm">
              <span><span className="font-semibold">{item.quantity}x</span> {item.name}</span>
              <span className="font-mono">{item.price.toLocaleString()} UZS</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          ...
          {/* Yandex Map Embed - hide for history items for performance */}
          {!isHistory && (
            <div className="w-full h-32 rounded-lg overflow-hidden bg-muted">
              <iframe
                src={`https://yandex.com/map-widget/v1/?mode=search&text=${encodeURIComponent(order.deliveryAddress)}&z=16&results=1`}
                width="100%"
                height="100%"
                frameBorder="0"
                allowFullScreen
        ...
        {order.deliveryPhone && (
          <div className="flex items-center gap-2">
            ...
            <a href={`tel:${order.deliveryPhone}`} className="text-sm text-primary hover:underline">
              {order.deliveryPhone}
            </a>
          </div>
        )}
        ...
        {isPending && onConfirm && (
          <div className="pt-2">
            <p className="text-xs text-orange-500 mb-2 font-medium">
              New order received! Please confirm to start preparing.
            </p>
        ...
        {!isPending && !isHistory && onUpdateStatus && order.status !== 'completed' && (
