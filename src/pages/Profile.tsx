...
export default function Profile() {
  ...
  const restaurantIds = useMemo(() => {
    ...
    return ids;
  }, [user.managedRestaurants, user.userId, getVideosByOwner]);
  ...
  const videoIds = useMemo(() => {
    return (user.restaurantVideos || []).map(v => v.id);
  }, [user.restaurantVideos]);
  
  const totalVideoViews = useMemo(() => {
    return getTotalViewsForVideos(videoIds);
  }, [videoIds, getTotalViewsForVideos]);
  
  const totalVideoLikes = useMemo(() => {
    return getTotalLikesForVideos(videoIds);
  }, [videoIds, getTotalLikesForVideos]);
  ...
  const handleAddRestaurantVideo = async () => {
    ...
    try {
      ...
      // Save video blob to IndexedDB for large file support
      ...
      const newVideo: SharedVideo = {
        id: videoId,
        videoUrl: videoId, // Store video ID, will be fetched from IndexedDB
      ...
      addSharedVideo(newVideo);
      // Also save to user's own store for profile display
  ...
  const handleAuth = async () => {
    if (authMode === 'register') {
      ...
      const registerSuccess = await user.register(authForm.name, authForm.email, authForm.password, selectedAccountType);
      if (!registerSuccess) {
          ...
          <div>
            <div className="grid grid-cols-3 gap-3">
              {(user.accountType === 'restaurant' ? [
                ...
                { label: 'Videos', value: formatNumber(user.restaurantVideos?.length || 0), icon: Play },
              ] : [
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {user.restaurantVideos?.length || 0} videos posted
                        </p>
                      </div>
                    </div>
                    <ChevronRight className="w-5 h-5 text-muted-foreground" />
        ...
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            {user.restaurantVideos && user.restaurantVideos.length > 0 ? (
              [...user.restaurantVideos].reverse().map((video) => (
                  ...
                  <div className="flex gap-3 p-3 w-full">
                    ...
                    <button
                      onClick={() => {
                        ...
                        deleteSharedVideo(video.id, user.userId);
