...
export default function Home() {
  ...
  // Фильтруем видео в зависимости от активной вкладки (memoized)
  const filteredVideos = useMemo(() => 
    activeTab === 'following' 
      ? videos.filter(video => followedRestaurants.includes(video.restaurantId))
      : videos,
    [activeTab, videos, followedRestaurants]
  );

  // Track video view when a video becomes active
  const currentVideoId = filteredVideos[currentIndex]?.id;
  useEffect(() => {
    if (currentVideoId) {
      markVideoAsViewed(currentVideoId);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentIndex, currentVideoId]);
  ...
  const handleLogoClick = () => {
    // Reset to first video and scroll to top
    ...
    <main className="relative h-screen w-full bg-black overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full px-6 py-4 flex justify-between items-center z-40">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
        >
          <span className="text-xl font-black tracking-tighter text-white drop-shadow-lg cursor-pointer hover:opacity-80 transition-opacity">
      ...
      <AnimatePresence>
        {isCartOpen && (
          <CartDrawer
            isOpen={isCartOpen}
            currentVideoId={filteredVideos[currentIndex]?.id}
            currentFoodItemId={filteredVideos[currentIndex]?.foodItemId}
      ...
      {/* Comments Overlay */}
      <CommentsOverlay
        videoId={filteredVideos[currentIndex]?.id || ''}
