...
export default function Home() {
  ...
  return (
    <main className="relative h-screen w-full bg-black overflow-hidden">
      ...
      {/* Branding Overlay (Centered at top) */}
      <div className="absolute top-12 left-0 right-0 z-40 pointer-events-none flex justify-center">
        <span className="text-2xl font-black tracking-tight text-white drop-shadow-lg flex items-baseline">
          <span>Order</span><span className="text-primary">Uz</span>
        </span>
      </div>

      {/* Top Navigation Tabs Overlay (Moved below logo) */}
      <div className="absolute top-24 left-0 w-full pb-6 px-6 flex justify-center items-center gap-6 z-40 pointer-events-none">
        <motion.button 
          onClick={() => setActiveTab('following')}
          className={`font-semibold text-base pointer-events-auto transition-colors relative ${
