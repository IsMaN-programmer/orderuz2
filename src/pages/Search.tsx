...
import { Search as SearchIcon, MapPin, Star, Play, Filter, Clock, ChevronRight, X, Check } from 'lucide-react';
...
// Restaurant type categories for filtering
...
export default function Search() {
  ...
  useEffect(() => {
    // Fetch all public restaurants created by any user
    api.restaurants.getAll().then(data => {
      setServerRestaurants(data);
    }).catch(console.error);
  }, []);
  ...
  // Combine mock restaurants with all serverside restaurants
  const allRestaurants = useMemo(() => {
    const apiRestaurants = serverRestaurants.map(rest => ({
      ...rest,
      id: rest.id,
      name: rest.name,
      logo: rest.logo,
      ...
      location: rest.address ? `${rest.address}, ${rest.city || 'Tashkent'}` : rest.location || 'Location not provided',
      description: rest.description || '',
      isManagedRestaurant: true
    } as any));
    
    // Filter out duplicates if any
    const uniqueApiRests = apiRestaurants.filter((rest, index, self) => 
      index === self.findIndex((r) => r.id === rest.id)
    );

    return [...uniqueApiRests, ...mockRestaurants];
  }, [serverRestaurants]);
  ...
  const filteredRestaurants = useMemo(() => {
    return allRestaurants.filter(res => {
      ...
      const matchesCategory = activeCategory === 'All' || res.category.includes(activeCategory);
      const matchesType = selectedTypes.length === 0 || selectedTypes.some(type => 
        res.category.toLowerCase().includes(type.toLowerCase())
      );
      return matchesSearch && matchesCategory && matchesType;
    });
  }, [searchQuery, activeCategory, allRestaurants, selectedTypes]);
    ...
    <div className="h-screen bg-background pb-24 overflow-y-auto flex flex-col">
      ...
      <Dialog open={isFilterDialogOpen} onOpenChange={setIsFilterDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Filter by Restaurant Type</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
