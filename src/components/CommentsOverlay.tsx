...
import { MessageCircle, Send, Trash2, Star } from 'lucide-react';
...
export const CommentsOverlay: React.FC<CommentsOverlayProps> = ({
  ...
  const { comments, addComment, deleteComment, getCommentsByVideoId, fetchCommentsByVideo } =
    useCommentsStore();
  const { userId, name, avatar, isAuthenticated, theme } = useUserStore();

  // Fetch comments from server when overlay opens
  useEffect(() => {
    if (isOpen && videoId) {
      fetchCommentsByVideo(videoId);
    }
  }, [isOpen, videoId, fetchCommentsByVideo]);
                          ...
                          <div className="flex gap-0.5">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <Star
                                key={star}
                                size={14}
                                className={star <= comment.rating ? 'fill-yellow-400 text-yellow-400' : isDarkMode ? 'text-white/20' : 'text-gray-300'}
                              />
                            ))}
                          </div>
