 ...
 */
export function CartDrawer({ isOpen, onClose, onOrderPlaced, currentVideoId, currentFoodItemId }: CartDrawerProps) {
  const { items, updateQuantity, removeItem, totalPrice, totalItems, isEmpty, clearCart } = useCart();
  const { t } = useTranslation();
  const isAuthenticated = useUserStore((state) => state.isAuthenticated);
  const location = useUserStore((state) => state.location);
  const userPhone = useUserStore((state) => state.phone);
  const userId = useUserStore((state) => state.userId);
  const incrementOrdersCount = useUserStore((state) => state.incrementOrdersCount);
  const createOrder = useOrderStore((state) => state.createOrder);
  const userAddress = location.district; // Get address from Yandex location
  ...
  const handlePlaceOrder = () => {
    ...
    if (!userAddress.trim()) {
      toast.error('Please set delivery address in Profile');
      return;
    }
    ...
    try {
      ...
      const restaurantName = firstItem.restaurantName || 'Restaurant';

      // Create order with address from Profile
      createOrder(
        restaurantId,
        restaurantName,
        items,
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent 
        side="right" 
        className="w-full flex flex-col p-0 bg-background/95 backdrop-blur-2xl border-l border-border/40 shadow-2xl [&_button[type='button']]:hidden"
      >
        {/* Header Section */}
          ...
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center h-[70vh] text-center px-10 space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold">{t('hungry')}</h3>
            ...
            {!showConfirm ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-end pt-1">
                    <div className="flex flex-col">
                      <span className="text-xs text-muted-foreground font-bold uppercase tracking-widest">{t('totalPayable')}</span>
                      <span className="text-2xl font-black text-primary font-mono">
                        {finalTotal.toLocaleString()} UZS
                      </span>
                    </div>
                    <div className="text-[10px] text-muted-foreground font-bold text-right leading-tight">
                      {t('vatIncluded')}<br />© 2026 ORDERUZ
                    </div>
                  </div>
                </div>
              ...
              <>
                <div className="space-y-4 bg-muted/30 p-4 rounded-xl">
                  <div>
                    <span className="text-xs text-muted-foreground font-bold">{t('deliveryAddress')}</span>
                    <p className="text-sm font-semibold text-foreground mt-1">{userAddress}</p>
                  </div>
                  <div>
                    <span className="text-xs text-muted-foreground font-bold">Phone</span>
                    <p className="text-sm font-semibold text-foreground mt-1">{userPhone}</p>
                  </div>
                  <Separator className="bg-border/30" />
                  <div className="pt-2">
                    <span className="text-xs text-muted-foreground font-bold">{t('totalPayable')}</span>
                    <span className="block text-2xl font-black text-primary font-mono mt-1">
                      {finalTotal.toLocaleString()} UZS
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button 
                    ...
                    className={cn(
                      "h-12 font-extrabold gap-2 rounded-xl",
                      "bg-primary text-primary-foreground shadow-lg shadow-primary/20",
                      "hover:translate-y-[-2px] active:scale-[0.98] transition-all duration-200"
                    )}
                  >
