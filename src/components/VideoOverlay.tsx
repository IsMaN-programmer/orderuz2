...
export function VideoOverlay({ video, onLike, onShare, onOrder, onComments }: VideoOverlayProps) {
  ...
  const [isLiked, setIsLiked] = useState(video.userHasLiked);
  const [isLikeUpdating, setIsLikeUpdating] = useState(false);

  useEffect(() => {
    setIsLiked(video.userHasLiked);
    setIsLikeUpdating(false);
  }, [video.id, video.userHasLiked]);
  ...
  const handleFollow = (e: React.MouseEvent) => {
    e.stopPropagation();
    const displayName = video.ownerName || video.restaurantName;
    if (isFollowing) {
      unfollowRestaurant(video.restaurantId);
      toast.info(`Вы отписались от ${displayName}`);
    } else {
      followRestaurant(video.restaurantId);
      toast.success(`Вы подписались на ${displayName}!`);
    }
  };
  ...
  return (
    <div className="absolute inset-0 z-10 pointer-events-none flex flex-col justify-end">
      {/* Interaction Sidebar */}
      <div
        className="absolute right-4 bottom-24 flex flex-col items-center gap-6 pointer-events-auto"
      >
        {/* Restaurant Avatar */}
        <motion.div
        ...
        >
          <Avatar className="h-12 w-12 border-2 border-white shadow-xl cursor-pointer">
            <AvatarImage src={video.ownerProfileImage || `https://api.dicebear.com/7.x/initials/svg?seed=${video.ownerName || video.restaurantName}`} />
            <AvatarFallback>{(video.ownerName || video.restaurantName).substring(0, 2)}</AvatarFallback>
          </Avatar>
          <motion.div 
      ...
      <div className="w-full p-6 pb-28 bg-gradient-to-t from-black/80 via-black/20 to-transparent pointer-events-auto">
        <div className="flex flex-col gap-4 max-w-[85%]">
          <div className="space-y-1">
            ...
            >
              @{video.ownerName || video.restaurantName}
            </motion.h3>
            <motion.p 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
