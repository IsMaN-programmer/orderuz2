  ...
  persist(
    (set, get) => ({
      ...
      login: (email: string, password: string, accountType?: AccountType) => {
        ...
        // Если accountType указан, проверяем что он совпадает
        if (account && accountType && account.accountType !== accountType) {
          return false;
        }
      ...
      register: (name: string, email: string, password: string, accountType: AccountType) => {
        ...
        state.login(email, password, accountType || 'user');
        return true;
      },
      ...
      incrementOrdersCount: () => {
        set((state) => ({ ordersCount: state.ordersCount + 1 }));
        setTimeout(() => get().updateCurrentAccount(), 0);
      },
