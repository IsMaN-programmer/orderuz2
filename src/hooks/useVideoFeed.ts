...
export const useVideoFeed = () => {
  ...
  useEffect(() => {
    ...
    const loadVideos = async () => {
      ...
      if (sharedVideos && sharedVideos.length > 0) {
        const sharedVideosConverted = sharedVideos.map((sharedVideo) => {
            ...
            // Video URL is stored as base64 data URL from server
            const videoUrl = sharedVideo.videoUrl;
  ...
  const toggleLike = useCallback(async (videoId: string): Promise<boolean | null> => {
